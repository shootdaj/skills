...
- `server.py`: `python3 server.py &` serves on 127.0.0.1:7331 (`--port` to choose). A copy already running is reused; if another program holds the port it moves to a free one and prints it: pass that as `data-api` on the vote.js tag and `?api=` on ballot.html. `votes.json` holds the latest pick per component, `votes.jsonl` the log. CORS is open for local `file://` pages.
