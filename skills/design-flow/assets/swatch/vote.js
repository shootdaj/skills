/* Design-comparison vote widget. Include with: <script src="../_vote/vote.js" data-design="d1" data-label="D1 Obsidian Spotlight"></script>
...
(function(){
  ...
  const KEY='sa-design-votes', API='http://127.0.0.1:7331'; let votes=load(), online=false, open=false;
  ...
  function render(){
    ...
    panel.innerHTML=`<button class="close" type="button" aria-label="Close">×</button><h4>${label}</h4><div class="sub">Overall pick so far: <b>${ovWho}</b> · ${online?'saved to Claude':'saved in browser only'}</div>
      ...
      <div style="font:800 11px system-ui;letter-spacing:.12em;color:#888;margin:4px 0 2px">PICK PER ELEMENT</div>${rows}
