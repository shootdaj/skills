/* Design-comparison vote widget. Include as the last line before </body>, after hub-bridge.js:
   <script src="../_vote/vote.js" data-design="d1" data-label="D1 Name"></script>
   Components, the storage key and the vote server address come from the room's room.js (window.ROOM), which this
   file loads from its own folder's parent. Picks are per component (radio across designs) plus overall. They persist
   to localStorage and to the local vote server when it is running. */
(function(){
  const me=document.currentScript||{dataset:{},src:''}; const design=me.dataset.design||'unknown'; const label=me.dataset.label||design;
  function start(){
    const R=window.ROOM||{}; const key=R.key||'room';
    const COMPONENTS=(R.components||[['overall','Overall design']]).map(x=>Array.isArray(x)?[x[0],x[1]]:[x.id,x.label]);
    if(!COMPONENTS.some(([c])=>c==='overall'))COMPONENTS.unshift(['overall','Overall design']);
    const KEY=key+'-design-votes', API=R.api||'http://127.0.0.1:7332'; let votes=load(), online=false, open=false;
    ...
    .sav .row span{flex:1}.sav .row small{display:block;color:#888;font-size:11.5px}
    ...
    function render(){
      ...
      panel.innerHTML=`<button class="close" type="button" aria-label="Close">×</button><h4>${label}</h4><div class="sub">Overall pick so far: <b>${ovWho}</b> · ${online?'saved to Claude':'saved in browser only'}</div>
        ...
        ${rows?`<div style="font:800 11px system-ui;letter-spacing:.12em;color:#888;margin:4px 0 2px">PICK PER ELEMENT</div>${rows}`:''}
  ...
  if(window.ROOM){start();return}
  let src='../room.js';try{src=new URL('../room.js',me.src||location.href).href}catch(e){}
  const s=document.createElement('script');s.src=src;s.onload=start;s.onerror=start;document.head.appendChild(s);
})();
