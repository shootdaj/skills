...
const launch = o => chromium.launchPersistentContext(profile, { headless: false, viewport: null, args: ['--start-maximized'], ...o });
    ...
    for (const r of q) {
      ...
      if (existsSync(p)) {
        try {
          ...
          for (const k of ['status', 'name', 'result', 'updated']) if (JSON.stringify(f[k]) !== JSON.stringify(r[k]) && f[k] !== undefined) { r[k] = f[k]; changed = true; }
        } catch (e) {}
      }
    }
    if (changed) {
