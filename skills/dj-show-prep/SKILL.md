---
name: dj-show-prep
description: End-to-end DJ gig preparation, run overnight and unattended after one round of questions — seed tracks from anywhere (Spotify, a screenshot, pasted text, a Lexicon playlist), expand with a similar-songs engine, acquire club-ready MP3s, verify and key them, build phased harmonically ordered playlists mixing new finds with the user's library, register in Lexicon, hand off to rekordbox/USB, publish a playable "vibe check" page. Use when the user says "I have a gig", "prep my set", "download this playlist and make it a set", "am I good for tonight", or sends a tracklist screenshot before a show. Composes dj-source-tracks, dj-soulseek-download, dj-set-builder, dj-lexicon-playlists, dj-vibe-report.
---

# dj-show-prep

The DJ wants to sleep and wake up ready. Ask **once** (§0), then run the whole
chain without further questions and report outcomes faithfully.
...
| LIBRARY | lexicon (+ rekordbox Modified sync) · m3u8-only · none | dj-lexicon-playlists |
...
## 0. Question round (one call, then autonomous)
...
5. **ANALYSE** (single): mixed-in-key (recommended) · keyfinder+aubio ·
   trust existing tags.
6. **LIBRARY** (single): lexicon + rekordbox sync (recommended) · m3u8 only ·
   none.
7. **REPORT** (single): vibe page compact (recommended) · full · chat only.
...
(never into chat) as the last question, then say goodnight.

## 1. Source → expand → CSV per phase  (`dj-source-tracks`)

Aim ~110 candidates per phase; 30–40 % will not be on Soulseek. Diff every
list against Lexicon (read-only) and earlier waves.
...
`set_builder.py`. Caps ~40/40/60 ≈ 11 h for a 3 h slot: right amount of choice.

## 4. Hand-off files

Flat copy of every keeper to `~/…/DJ/<Gig> <Date>/` (copy, never move) for
the Lexicon drag; `.m3u8` per phase with absolute paths.

## 5. Library → export  (`dj-lexicon-playlists`)
...
Publish, copy URL to clipboard. Final message = URL, playlist table (name,
slot, tracks, BPM range), file locations, what could not be obtained and why,
